export default async (req) => {
  ...
  const apiUrl =
    Netlify.env.get('MIMO_API_URL') ||
    'https://api.xiaomimimo.com/v1/chat/completions';
  const model = Netlify.env.get('MIMO_MODEL') || 'mimo-v2-flash';
  ...
  try {
    ...
    const reply =
      data.choices?.[0]?.message?.content || data.data?.content || '（沉默）';
